...
export default {
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            if (path === '/view.js') {
                return new Response(VIEW_SCRIPT, {
                    headers: { 'Content-Type': 'application/javascript', ...corsHeaders }
                });
            }
...
async function handleCreatePaste(request, env, corsHeaders) {
    ...
    const pasteData = {
        ...
        content: data.content, // This is now encrypted content from client
        language: data.language || 'text',
        createdAt,
        expiresAt,
        views: 0,
        hasPassword: !!data.password,
        encrypted: data.encrypted || false // Flag to indicate if content is encrypted
    };
...
async function handleGetPaste(request, env, pasteId, corsHeaders) {
    ...
    // Return paste data (content remains encrypted if it was encrypted)
...
async function handleViewPaste(request, env, pasteId) {
    ...
    // For encrypted content, we need to serve a page that can handle decryption
    if (paste.encrypted) {
        // Serve the view page with decryption capabilities
        const html = VIEW_HTML
            .replace(/{{TITLE}}/g, paste.title || 'Untitled')
            .replace(/{{LANGUAGE}}/g, paste.language || 'text')
            .replace(/{{CONTENT}}/g, '') // Content will be loaded via API and decrypted client-side
        ...
        // For non-encrypted content, render normally
        const html = VIEW_HTML
            .replace(/{{TITLE}}/g, paste.title || 'Untitled')
            .replace(/{{LANGUAGE}}/g, paste.language || 'text')
